package org.tecklens.splkeycloak.providers.keycloak.phone.authentication.authenticators.resetcred;
...
public class ResetCredentialWithPhone implements Authenticator, AuthenticatorFactory {
  ...
  protected boolean validateForm(AuthenticationFlowContext context, MultivaluedMap<String, String> inputData) {
    boolean byPhone = OptionalUtils
        .ofBlank(inputData.getFirst(SupportPhonePages.FIELD_PATH_PHONE_ACTIVATED))
        .map(s -> "true".equalsIgnoreCase(s) || "yes".equalsIgnoreCase(s))
        .orElse(false);

    context.clearUser();

    String phoneNumber = inputData.getFirst(SupportPhonePages.FIELD_PHONE_NUMBER);
    ...
    if (!byPhone){
      ...
      if (Validation.isBlank(phoneNumber)) {
        context.getEvent().error(Errors.USERNAME_MISSING);
        Response challenge = challenge(context, SupportPhonePages.FIELD_PHONE_NUMBER, SupportPhonePages.Errors.MISSING.message(), phoneNumber);
      ...
      try {
        ...
        context.getEvent().error(Errors.USERNAME_MISSING);
        Response challenge = challenge(context, SupportPhonePages.FIELD_PHONE_NUMBER,
      ...
      String verificationCode = inputData.getFirst(SupportPhonePages.FIELD_VERIFICATION_CODE);
  ...
  protected boolean isDisabledByBruteForce(AuthenticationFlowContext context, UserModel user, String phoneNumber) {
    ...
    if (bruteForceError != null) {
      ...
      Response challenge = challenge(context, SupportPhonePages.FIELD_PHONE_NUMBER, Messages.INVALID_USER, phoneNumber);
      context.forceChallenge(challenge);
      return true;
    }
    return false;
  ...
  protected boolean isDisabledByBruteForce(AuthenticationFlowContext context, UserModel user) {
    ...
    if (bruteForceError != null) {
      context.getEvent().user(user);
      context.getEvent().error(bruteForceError);
      Response challenge = challenge(context, SupportPhonePages.FIELD_PHONE_NUMBER, Messages.INVALID_USER);
      context.forceChallenge(challenge);
      return true;
    }
    return false;
  ...
  protected boolean validateUser(AuthenticationFlowContext context,
    ...
    if (user == null){
      ...
      Response challenge = byPhone ? challenge(context, SupportPhonePages.FIELD_PHONE_NUMBER, SupportPhonePages.Errors.USER_NOT_FOUND.message(), attempted) : challenge(context, Validation.FIELD_USERNAME, Messages.INVALID_USERNAME_OR_EMAIL);
      context.forceChallenge(challenge);
      return false;
    }
    ...
    if (!user.isEnabled()) {
      ...
      Response challenge = byPhone ? challenge(context, SupportPhonePages.FIELD_PHONE_NUMBER, Errors.USER_DISABLED, attempted) : challenge(context,Validation.FIELD_USERNAME,Errors.USER_DISABLED);
      context.forceChallenge(challenge);
      return false;
    }
  ...
  protected void invalidVerificationCode(AuthenticationFlowContext context, String phoneNumber) {
    ...
    Response challenge = challenge(context, SupportPhonePages.FIELD_VERIFICATION_CODE, SupportPhonePages.Errors.NOT_MATCH.message(), phoneNumber);
  ...
  protected Response challenge(AuthenticationFlowContext context,
    ...
    return context.form()
        ...
        .setAttribute(SupportPhonePages.ATTRIBUTE_SUPPORT_PHONE, true)
        .setAttribute(SupportPhonePages.ATTEMPTED_PHONE_ACTIVATED, true)
