package org.tecklens.splkeycloak.providers.keycloak.phone.authentication.forms;
...
public class RegistrationPhoneVerificationCode implements FormAction, FormActionFactory {
  ...
  public void validate(ValidationContext context) {
    ...
    String phoneNumber = formData.getFirst(SupportPhonePages.FIELD_PHONE_NUMBER);

    if (Validation.isBlank(phoneNumber)){
      context.error(Errors.INVALID_REGISTRATION);
      errors.add(new FormMessage(SupportPhonePages.FIELD_PHONE_NUMBER, SupportPhonePages.Errors.MISSING));
    ...
    try {
      ...
      context.error(Errors.INVALID_REGISTRATION);
      errors.add(new FormMessage(SupportPhonePages.FIELD_PHONE_NUMBER, e.getErrorType().message()));
      context.validationError(formData, errors);
      return;
    }

    context.getEvent().detail(SupportPhonePages.FIELD_PHONE_NUMBER, phoneNumber);

    String verificationCode = formData.getFirst(SupportPhonePages.FIELD_VERIFICATION_CODE);
    ...
    if (Validation.isBlank(verificationCode) || tokenCode == null || !tokenCode.getCode().equals(verificationCode)) {
      context.error(Errors.INVALID_REGISTRATION);
      formData.remove(SupportPhonePages.FIELD_VERIFICATION_CODE);
      errors.add(new FormMessage(SupportPhonePages.FIELD_VERIFICATION_CODE, SupportPhonePages.Errors.NOT_MATCH.message()));
      context.validationError(formData, errors);
      return;
    }
