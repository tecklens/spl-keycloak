package org.tecklens.splkeycloak.providers.keycloak.phone.providers.spi.impl;
...
import org.tecklens.splkeycloak.providers.keycloak.phone.providers.representations.TokenCodeRepresentation;
import org.tecklens.splkeycloak.providers.keycloak.phone.providers.spi.MessageSenderService;
import org.tecklens.splkeycloak.providers.keycloak.phone.providers.spi.PhoneProvider;
...
public class DefaultPhoneProvider implements PhoneProvider {
    ...
    DefaultPhoneProvider(KeycloakSession session, Scope config) {
        ...
        this.targetHourMaximum = config.getInt("targetHourMaximum", 1000);
        this.sourceHourMaximum = config.getInt("sourceHourMaximum", 1000);
    }
    ...
    private Optional<String> getStringConfigValue(String configName, String def) {
        return OptionalUtils.ofBlank(OptionalUtils.ofBlank(config.get(getRealmName() + "-" + configName))
                .orElse(config.get(configName, def)));
    }
    ...
    public int otpExpires() {
        return getStringConfigValue("otp-expires").map(Integer::valueOf).orElse(2 * 60 * 60); // * 2 phút
    }
    ...
    public Optional<String> defaultPhoneRegion() {
        return getStringConfigValue("phone-default-region", "VN");
    }
    ...
    @Override
    public int sendTokenCode(String phoneNumber, String sourceAddr, TokenCodeType type, String kind, String message) {
        ...
        if (getTokenCodeService().isAbusing(phoneNumber, type, sourceAddr, sourceHourMaximum, targetHourMaximum)) {
            throw new ForbiddenException("Bạn yều cầu vượt quá số lần gửi OTP trong giờ qua");
        }
        ...
        try {
            session.getProvider(MessageSenderService.class, service).sendSmsMessage(type, phoneNumber, token.getCode(), tokenExpiresIn, kind, message);
