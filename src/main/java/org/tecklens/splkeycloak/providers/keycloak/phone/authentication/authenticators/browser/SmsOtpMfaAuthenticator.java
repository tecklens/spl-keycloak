...
import org.tecklens.splkeycloak.providers.keycloak.phone.authentication.requiredactions.ConfigSmsOtpRequiredAction;
...
import static org.tecklens.splkeycloak.providers.keycloak.phone.authentication.forms.SupportPhonePages.ATTRIBUTE_SUPPORT_PHONE;
...
public class SmsOtpMfaAuthenticator implements Authenticator, CredentialValidator<PhoneOtpCredentialProvider> {
    ...
    private String getCredentialMessage(UserModel user) {
        return PhoneOtpCredentialModel.getSmsOtpCredentialData(user)
                .map(PhoneOtpCredentialModel.SmsOtpCredentialData::getMessage)
                .orElseThrow(() -> new IllegalStateException("Not have Message OTP Credential"));
    }
    ...
    public void authenticate(AuthenticationFlowContext context) {
        ...
        String message = getCredentialMessage(context.getUser());

        boolean verified = OptionalUtils.ofBlank(context.getAuthenticationSession().getAuthNote(PhoneUsernamePasswordForm.VERIFIED_PHONE_NUMBER))
        ...
        try {
            int expires = phoneProvider.sendTokenCode(phoneNumber, context.getConnection().getRemoteAddr(),
                    TokenCodeType.OTP, null, message);
            context.form()
