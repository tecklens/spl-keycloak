package org.tecklens.splkeycloak.providers.keycloak.phone.credential;
...
public class PhoneOtpCredentialModel extends CredentialModel {
    ...
    public static Optional<SmsOtpCredentialData> getSmsOtpCredentialData(@NotNull UserModel user){
        return getOtpCredentialModel(user)
            .map(credentialModel -> {
                try {
                    return JsonSerialization.readValue(credentialModel.getCredentialData(), SmsOtpCredentialData.class);
                } catch (IOException e) {
...
//        @ConstructorProperties("phoneNumber")
        public SmsOtpCredentialData(@JsonProperty("phoneNumber") String phoneNumber,
            ...
            this.phoneNumber = phoneNumber;
            this.secretCreate = new Date().getTime();
            this.expires = expires == null ? 60 : expires;
            this.message = "AIO: QK nhap ma OTP %s de dang nhap tai khoan tren ung dung AIO. Ma xac thuc co hieu luc trong vong %s phut. LH 1900989868 (1000VND/phut).";
        }
    }
