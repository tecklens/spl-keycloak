package org.tecklens.splkeycloak.providers.keycloak.phone;
...
//        List<Optional<UserModel>> users = new ArrayList<>();
//
//        for (String number : numbers) {
//            UserModel oUser = userProvider
//                    .getUserByUsername(realm, number);
//            users.add(Optional.of(oUser));
//        }
//
//        return users.get(0);

        logger.info(numbers.toArray());

        return numbers.stream().map(number -> {
                    UserModel u = userProvider
                            .getUserByUsername(realm, number);
                    logger.info(u);
                    return u;
                })
